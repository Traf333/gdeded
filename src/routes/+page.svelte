...
<section>
	...
	<div class="flex flex-col mb-3 xl:w-1/3 md:mx-auto md:w-2/3 xs:w-full p-3">
		{#each scenarios.reverse() as play (play)}
			<Card class="mb-3">
				<div class="flex justify-between w-full">
					<a
						href="/play/{play._id}"
						class="mb-2 flex-1 text-l font-semibold tracking-tight text-gray-900 dark:text-white"
					>
						{play.title}
					</a>
					<ToolbarButton
						class="dots-menu{play._id} text-gray-900 bg-white dark:text-white dark:bg-gray-800"
					>
						<svg
...
<Modal
	bind:open={selectedItem}
	title={selectedItem?._id ? 'Редактировать спектакль' : 'Создать спектакль'}
>
	<ScenarioForm
